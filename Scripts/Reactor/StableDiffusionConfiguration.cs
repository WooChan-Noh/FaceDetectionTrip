...
public class StableDiffusionConfiguration : MonoBehaviour
{
    ...
    void Start()//귀찮아서 내가  작성함
    {
        Debug.LogWarning("URL 주소 갱신했는지 확인할 것. URL 마지막에 \\이 없는지 확인할 것. 있으면 오류남");
        if (settings.StableDiffusionServerURL!=" ")
        {
            ListModels();
            Debug.Log("스테이블 디퓨전 모델 로드 완료");

        }
        else
            Debug.LogError("URL 주소가 비어있음");
    }
    ...
    /// </summary>
    /// <returns></returns>
    IEnumerator ListModelsAsync()//에러로그 한글로 수정
    {
        ...
        catch (Exception)
        {
            ...
            Debug.LogError("잘못된 URL주소! 또는 API 인증 키가 잘못됨 - settings 확인할 것");
